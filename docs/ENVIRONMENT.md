...
`DEPLOYMENT_ENV` is web build/server configuration: use `staging` or `production` on connected deployments. `PITCH_PREVIEW_BUILD=1` is an explicit non-connected local/CI review flag; never use it on deployments. Keep it set when starting a local production review build. The web build validates every required public/server origin and rejects privileged browser keys. API origins must be HTTPS origins in production; restricted `rk_live_` keys are also prohibited outside production.

The old live-test email/password defaults are no longer used. `STAGING.md` lists the required `E2E_*` variables; tests create unique confirmed fictional identities with random passwords. All E2E secrets belong only to the restricted test runner. `E2E_STRIPE=1` adds TEST-only payment tests and requires test API/signing secrets.

Business contact is database configuration: `business_settings.value.business_email` and `business_phone` (E.164, e.g. a leading `+` and country code). Configure through the administrator Settings page. Blank contact prevents composer links and shows a configuration notice. No personal contact is hardcoded in feature modules.
